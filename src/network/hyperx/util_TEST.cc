...
class TestRouter : public Router {
 public:
  TestRouter(const std::vector<u32>& _address, u32 _numPorts, u32 _numVcs,
      ...
      : Router("TestRouter " + strop::vecString<u32>(_address), nullptr, 0,
               _address, _numPorts, _numVcs, nullptr,
...
void intNodeTestCongestion(
    ...
    u32 _conc, u32 _vcSet, u32 _numVcSets, u32 _numVcs,
    u32 _refNumBuckets,
  ...
  for (u64 idx = 0; idx < kRounds; idx++) {
    ...
    _intNodeAlgFunc(router, _sourceRouter, _destinationTerminal, _dimWidths,
                    _dimWeights, _conc, _vcSet, _numVcSets, _numVcs, &addr);
    u32 id = Cube::computeTerminalId(&addr, _dimWidths, 1);
...
void singleRoutingTest(
    ...
    u32 _conc, u32 _vcSet, u32 _numVcSets, u32 _numVcs,
    const std::unordered_set< std::tuple<u32, u32, f64> > _refOutputPorts,
    const std::unordered_map<u32, f64>& _congStatus,
  ...
  router = new TestRouter(_sourceRouter, numPorts, _numVcs, _congStatus);
  _routingAlgFunc(router, _widths, _weights, _conc, _destinationTerminal,
                  _vcSet, _numVcSets, _numVcs, &outputPorts);
...
void distRoutingTest(
  ...
  for (u64 idx = 0; idx < kRounds; idx++) {
    if (_routingAlgFunc) {
      _routingAlgFunc(router, _widths, _weights, _conc, _destinationTerminal,
                      _vcSet, _numVcSets, _numVcs, &vcPool);
    } else {
      _firstHopAlgFunc(router, _widths, _weights, _conc, _destinationTerminal,
                       _vcSet, _numVcSets, _numVcs, _shortCut,
                       &vcPool);
    }
...
TEST(HyperXUtil, intNodeUnAligned) {
  TestSetup ts(1, 1, 0xBAADF00D);
  std::vector<u32> src;
  std::vector<u32> dst;
...
TEST(HyperXUtil, valiantsRoutingAlgorithm) {
  ...
  HyperX::valiantsRoutingOutput(router, widths, weights, conc, &dst,
