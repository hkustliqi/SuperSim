...
AverageProgressiveAdaptiveRoutingAlgorithm::
  AverageProgressiveAdaptiveRoutingAlgorithm(
...
void AverageProgressiveAdaptiveRoutingAlgorithm::processRequest(
  ...
  if (ri->valiantMode == false) {
    outputPorts = AverageProgressiveAdaptiveRoutingAlgorithm::routing
...
std::unordered_set<u32> AverageProgressiveAdaptiveRoutingAlgorithm::routing(
  ...
  if (ri->globalHopCount == 0 && globalDim != globalDimensions) {
    ...
    if (MINAvailability * MINPathLen <=
        AverageAvailability * (NonMINPathLen + 1)) {
...
u32 AverageProgressiveAdaptiveRoutingAlgorithm::setIntermediateAdd(
