...
ProgressiveAdaptiveAverageRoutingAlgorithm::
  ProgressiveAdaptiveAverageRoutingAlgorithm(
...
void ProgressiveAdaptiveAverageRoutingAlgorithm::processRequest(
  ...
  if (ri->valiantMode == false) {
    outputPorts = ProgressiveAdaptiveAverageRoutingAlgorithm::routing
...
std::unordered_set<u32> ProgressiveAdaptiveAverageRoutingAlgorithm::routing(
  ...
  if (ri->globalHopCount == 0 && globalDim != globalDimensions) {
    ...
    if (MINAvailability * MINPathLen <=
        AverageAvailability * NonMINPathLen) {
...
u32 ProgressiveAdaptiveAverageRoutingAlgorithm::setIntermediateAdd(
