 ...
 */
#include "traffic/DragonflyGlobalWorstCaseTrafficPattern.h"
...
TEST(DragonflyGlobalWorstCaseTrafficPattern, permutation) {
  for (u32 net = 0; net < 100; net++) {
    ...
    for (u32 test = 0; test < 100; test++) {
      ...
      DragonflyGlobalWorstCaseTrafficPattern* tp =
          new DragonflyGlobalWorstCaseTrafficPattern(
              "TP", nullptr, groupCount * groupSize * concentration, self,
              settings);

      u32 expDestGroup = groupCount - selfGroup - 1;
...
TEST(DragonflyGlobalWorstCaseTrafficPattern, random) {
